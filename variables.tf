...
variable "diagnostic_settings" {
  ...
  - `log_categories` - (Optional) A set of log categories to send to the log analytics workspace. Defaults to `[]`. If this is non-empty, `log_groups` is ignored.
  - `log_groups` - (Optional) A set of log groups to send to the log analytics workspace. Defaults to `["allLogs"]`. Used only when `log_categories` is empty.
  ...
  validation {
    condition     = alltrue(flatten([for _, v in var.diagnostic_settings : [for c in v.log_categories : contains(["AppAvailabilityResults", "AppBrowserTimings", "AppEvents", "AppMetrics", "AppDependencies", "AppExceptions", "AppPageViews", "AppPerformanceCounters", "AppRequests", "AppSystemEvents", "AppTraces", "OTelResources"], c)]]))
    error_message = "Each log category must be one of: AppAvailabilityResults, AppBrowserTimings, AppEvents, AppMetrics, AppDependencies, AppExceptions, AppPageViews, AppPerformanceCounters, AppRequests, AppSystemEvents, AppTraces, OTelResources."
  }
  validation {
    condition     = alltrue([for _, v in var.diagnostic_settings : v.workspace_resource_id == null || v.workspace_resource_id != var.workspace_id])
    error_message = "Each `workspace_resource_id` in `diagnostic_settings` must be different from `workspace_id`."
  }
  validation {
    condition     = alltrue([for _, v in var.diagnostic_settings : (v.workspace_resource_id != null || v.storage_account_resource_id != null || v.event_hub_authorization_rule_resource_id != null || v.marketplace_partner_resource_id != null)])
    error_message = "Each item in `diagnostic_settings` must define at least one destination (`workspace_resource_id`, `storage_account_resource_id`, `event_hub_authorization_rule_resource_id`, or `marketplace_partner_resource_id`)."
  }
}
